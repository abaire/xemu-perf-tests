...
class TestSuiteDescriptor(NamedTuple):
    """Describes one of the test suites."""
...
class TestSuiteDescriptorReader:
    ...
    def process(self) -> list[TestSuiteDescriptor]:
        """Loads the test suite descriptors from the project."""
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        help="Directory into which the tests_registry.json file will be generated (defaults to xml_dir)",
    )
    ...
    output_file = os.path.join(args.output_dir, "tests_registry.json")
