...
static constexpr char kMixedVertexCountTest[] = "MixedVtxCount";

static constexpr uint32_t kMixedVertexCounts[] = {
    0x100, 0x01FFFF, 0x030000, 0x040000, 0x07FFFF,
};
static constexpr uint32_t kNumVertexCounts = sizeof(kMixedVertexCounts) / sizeof(kMixedVertexCounts[0]);
...
HighVertexCountTests::HighVertexCountTests(TestHost& host, std::string output_dir, const Config& config)
  ...
  for (auto draw_mode : {DRAW_ARRAYS, DRAW_INLINE_BUFFERS, DRAW_INLINE_ARRAYS, DRAW_INLINE_ELEMENTS}) {
    ...
    name = MakeTestName(kMixedVertexCountTest, draw_mode);
...
static void CreateGeometry(TestHost &host, std::shared_ptr<VertexBuffer> &vertex_buffer,
                           std::vector<uint32_t> &index_buffer, uint32_t target_array_entries = 0x07FFFF) {
  vertex_buffer.reset();
  index_buffer.clear();
...
void HighVertexCountTests::Initialize() {
  ...
  for (auto i = 0; i < kNumVertexCounts; ++i) {
    ...
    CreateGeometry(host_, entry.vertex_buffer, entry.index_buffer);
  }
}
...
void HighVertexCountTests::Test(const std::string& name, DrawMode draw_mode) {
  ...
  switch (draw_mode) {
    ...
    case DRAW_INLINE_ELEMENTS:
      results = Profile(name, 50, [this, &attributes, &index_buffer] {
...
void HighVertexCountTests::TestMixedSizes(const std::string &name, HighVertexCountTests::DrawMode draw_mode) {
  ...
  switch (draw_mode) {
    case DRAW_ARRAYS:
      results = Profile(name, 50, [this, &attributes] {
        for (auto i = 0; i < kNumVertexCounts; ++i) {
          auto &vertex_buffer = geometry_[i].vertex_buffer;
          host_.SetVertexBuffer(vertex_buffer);
    ...
    case DRAW_INLINE_BUFFERS:
      results = Profile(name, 2, [this, &attributes] {
        for (auto i = 0; i < kNumVertexCounts; ++i) {
          auto &vertex_buffer = geometry_[i].vertex_buffer;
          host_.SetVertexBuffer(vertex_buffer);
    ...
    case DRAW_INLINE_ELEMENTS:
      results = Profile(name, 10, [this, &attributes] {
        for (auto i = 0; i < kNumVertexCounts; ++i) {
          auto &vertex_buffer = geometry_[i].vertex_buffer;
          auto &index_buffer = geometry_[i].index_buffer;
          host_.SetVertexBuffer(vertex_buffer);
          host_.DrawInlineElements16(index_buffer, attributes, kPrimitive);
        }
      });
    ...
    case DRAW_INLINE_ARRAYS:
      results = Profile(name, 4, [this, &attributes] {
        for (auto i = 0; i < kNumVertexCounts; ++i) {
          auto &vertex_buffer = geometry_[i].vertex_buffer;
          host_.SetVertexBuffer(vertex_buffer);
